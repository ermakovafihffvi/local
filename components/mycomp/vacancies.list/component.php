...
if($arParams["IBLOCK_ID"] > 0 && $this->StartResultCache(false, ($arParams["CACHE_GROUPS"]==="N"? false: $USER->GetGroups())))
{
	...
	foreach($arSec as $secItem){
		$arResult['ITEMS'][$secItem['ID']]['ID'] = $secItem['ID'];
		$arResult['ITEMS'][$secItem['ID']]['SECTION_NAME'] = $secItem['NAME'];
		$arResult['ITEMS'][$secItem['ID']]['VACANCIES'] = [];
		foreach($arVacancies as $vacItem){
			if($vacItem['IBLOCK_SECTION_ID'] == $secItem['ID']){
				...
				$vacItem["EDIT_LINK"] = $arButtons["edit"]["edit_element"]["ACTION_URL"];
				$vacItem["DELETE_LINK"] = $arButtons["edit"]["delete_element"]["ACTION_URL"];

				$arResult['ITEMS'][$secItem['ID']]['VACANCIES'][] = $vacItem;

				$arResult["LAST_ITEM_IBLOCK_ID"] = $vacItem["IBLOCK_ID"];
			}
		}
	}
    file_put_contents(__DIR__."/arButtons.txt", print_r($arButtons, true));
    if($arResult)
    {
